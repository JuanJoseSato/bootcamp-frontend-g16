...
function getCoins(change) {
  ...
  let acc = change
  return coins
    .reverse() // Invierto la cadena para poder revisarlo de manera descendente
    .map(function (coin) { // El método map() crea un nuevo array con los resultados de la llamada a la función indicada aplicados a cada uno de sus elementos.
      let quotient = Math.floor(acc / coin) // Math.floor devuelve el máximo entero menor o igual al número
